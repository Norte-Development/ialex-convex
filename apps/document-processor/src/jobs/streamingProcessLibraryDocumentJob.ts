  ...
  new Worker<LibraryJobPayload>(
    ...
    async (job) => {
      ...
      try {
        ...
        // Extract text callback for transcription (audio/video files)
        const extractedTextCallback = async (
          extractedText: string,
          metadata: Record<string, unknown>,
        ) => {
          logger.info(
            "Storing full transcript to database (Library Document)",
            {
              libraryDocumentId: payload.libraryDocumentId,
        ...
        if (isFinalAttempt && process.env.AUTO_CLEANUP_ON_FAILURE !== "false") {
          logger.info("Cleaning up temp files (final attempt failed)", {
            jobId: job.id,
          });
          await pipeline.cleanup(true);
