...
import { splitPdfForMistralOCR, getPdfPageCount, uploadPdfChunk } from "./documentSplittingService";
...
async function processPdfChunk(chunkBuffer: Buffer, chunkIndex: number, totalChunks: number): Promise<string> {
  try {
    ...
    // Upload chunk to temporary storage and get URL
    const chunkUrl = await uploadPdfChunk(chunkBuffer, chunkIndex);
...
export async function extractWithMistralOCRFromBuffer(buffer: Buffer, opts: Options): Promise<string> {
  ...
  try {
    ...
    if (pageCount <= 500) {
      ...
      // Upload the buffer and process normally
      const documentUrl = await uploadPdfChunk(buffer, 0);
      return await extractWithMistralOCR(documentUrl, opts);
    }
