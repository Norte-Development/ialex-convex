...
export const getModelos = query({
  ...
  handler: async (ctx, args) => {
    ...
    let filteredModelos = allModelos.filter(
      (m) => m.isPublic || m.createdBy === currentUser._id,
    );

    // Apply additional filters
    if (args.category) {
      filteredModelos = filteredModelos.filter(
        (m) => m.category === args.category,
      );
    }

    if (args.content_type) {
      filteredModelos = filteredModelos.filter(
        (m) => m.content_type === args.content_type,
      );
    }

    if (args.isPublic !== undefined) {
      filteredModelos = filteredModelos.filter(
        (m) => m.isPublic === args.isPublic,
      );
    }
...
export const getModelo = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !modelo.isPublic &&
      modelo.createdBy !== currentUser._id &&
      modelo.createdBy !== "system"
    ) {
...
/**
 * Updates an existing template (modelo).
 *
 * @param {Object} args - The function arguments
 * @param {string} args.modeloId - The ID of the template to update
 * @param {string} [args.name] - New template name
 * @param {string} [args.description] - New description
 * @param {string} [args.category] - New category
 * @param {string} [args.content] - New template content
 * @param {string} [args.content_type] - New content type
 * @param {boolean} [args.isPublic] - New visibility setting
 * @param {string[]} [args.tags] - New tags array
 * @returns {Promise<null>}
 * @throws {Error} When not authenticated, template not found, or unauthorized
 *
 * @description This function updates an existing template. Only the creator
 * of the template can update it. System templates cannot be modified.
 *
 * @example
 * ```javascript
 * await updateModelo({
 *   modeloId: "template_123",
 ...
 * ```
 */
export const updateModelo = mutation({
  args: {
    modeloId: v.id("modelos"),
  ...
  handler: async (ctx, args) => {
    ...
    const existingModelo = await ctx.db.get(args.modeloId);

    if (!existingModelo) {
      throw new Error("Template not found");
    }

    // Prevent updates to system templates
    if (existingModelo.createdBy === "system") {
      throw new Error("System templates cannot be modified");
    }

    // Only creator can update
    if (existingModelo.createdBy !== currentUser._id) {
    ...
    if (args.content_type !== undefined)
      updateData.content_type = args.content_type;
    if (args.isPublic !== undefined) updateData.isPublic = args.isPublic;
    if (args.tags !== undefined) updateData.tags = args.tags;

    await ctx.db.patch(args.modeloId, updateData);
...
/**
 * Deletes (soft delete) a template (modelo).
 *
 * @param {Object} args - The function arguments
 * @param {string} args.modeloId - The ID of the template to delete
 * @returns {Promise<null>}
 * @throws {Error} When not authenticated, template not found, or unauthorized
 *
 * @description This function performs a soft delete on a template by setting
 * isActive to false. Only the creator of the template can delete it.
 * System templates cannot be deleted.
 *
 * @example
 * ```javascript
 * await deleteModelo({ modeloId: "template_123" });
 * ```
 */
export const deleteModelo = mutation({
  args: {
    modeloId: v.id("modelos"),
  },
  returns: v.null(),
  handler: async (ctx, args) => {
    ...
    if (modelo.createdBy === "system") {
      throw new Error("System templates cannot be deleted");
    }

    // Only creator can delete
    if (modelo.createdBy !== currentUser._id) {
  ...
  returns: v.union(
    v.object({
      ...
      templateType: v.optional(
        v.union(v.literal("escrito"), v.literal("document")),
      ),
      content: v.optional(v.string()),
      content_type: v.optional(v.string()),
...
export const internalSearchModelos = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // Filter to show only public templates OR private templates created by current user
    let filteredModelos = allModelos.filter(
      (m) => m.isPublic || m.createdBy === args.userId,
    );
