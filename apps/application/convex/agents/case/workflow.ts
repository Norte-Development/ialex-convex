...
export const streamWithContextAction = internalAction({
  ...
  handler: async (ctx, { threadId, promptMessageId, contextBundle }) => {
    ...
    const openRouterModel = modelToUse === 'gpt-5' ? 'z-ai/glm-4.6' : 'z-ai/glm-4.6';
    const reasoning = { reasoning: {enabled: true, effort: "low" as const, exclude: false } };
      ...
      await thread.streamText(
        {
          ...
          promptMessageId,
          model: openrouter(openRouterModel, {...reasoning,  provider: {order: ["z-ai","fireworks", "together"]}}),
          onAbort: () => {
