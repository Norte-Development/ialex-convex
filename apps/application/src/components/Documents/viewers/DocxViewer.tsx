  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const result = await mammothAny.convertToHtml({ arrayBuffer }, {
  ...
  if (error) {
    return (
      <div className="p-6 text-center">
        <div className="text-gray-700 mb-2">No se pudo renderizar el archivo DOCX.</div>
        <a
          href={url}
          target="_blank"
          rel="noreferrer"
  ...
  return (
    <div className="w-full h-full overflow-auto bg-gray-100" aria-label={title}>
      <style>{`
        ...
        .docx-page table { width: 100%; border-collapse: collapse; }
        ...
        .docx-page p { margin: 0.5em 0; }
        .docx-page ul, .docx-page ol { padding-left: 1.5em; }
      `}</style>
      <div className="docx-page prose prose-neutral max-w-none" dangerouslySetInnerHTML={{ __html: html }} />
    </div>
  );
}
