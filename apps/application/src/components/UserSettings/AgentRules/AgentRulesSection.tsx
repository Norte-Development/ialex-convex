...
export function AgentRulesSection() {
  const userRules = useQuery(api.functions.agentRules.getUserRules, { activeOnly: false } as any);
  ...
  const orderedRules = useMemo(() => {
    if (!userRules) return [];
    return [...userRules].sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0));
  }, [userRules]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">Reglas del Agente (Usuario)</h2>
      ...
      {orderedRules && orderedRules.length > 0 ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">No hay reglas aún</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Cree su primera regla para personalizar cómo el agente responde.
          </CardContent>
        </Card>
      )}
      ...
      <p className="text-xs text-muted-foreground">
        Las reglas de usuario siempre se aplican. También pueden existir reglas específicas por caso, que se combinan automáticamente cuando trabaja dentro de un caso.
      </p>
    </div>
  );
}
