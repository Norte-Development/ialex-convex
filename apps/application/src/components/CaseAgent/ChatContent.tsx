...
export function ChatContent() {
  ...
  }, [messages.results?.length, threadId, scrollState.isInitialLoad, scrollToBottom])
  ...
  useEffect(() => {
    if (messagesContainerRef.current && messages.results?.length) {
      ...
      if (currentCount > prevCount && !scrollState.isInitialLoad && prevCount > 0) {
        // Only auto-scroll if user hasn't manually scrolled away or is near bottom
        if (scrollState.shouldAutoScroll && (isNearBottom() || !scrollState.userHasScrolled)) {
          scrollToBottom()
        }
      }
  ...
  const handleLoadMore = useCallback(async () => {
    if (!messages.loadMore || scrollState.isLoadingMore || !messagesContainerRef.current) return
    ...
    (e: React.UIEvent<HTMLDivElement>) => {
      ...
      requestAnimationFrame(() => {
        ...
        scrollTimeoutRef.current = setTimeout(() => {
          ...
          if (scrollTop < 100 && !scrollState.isLoadingMore && messages.status === "CanLoadMore") {
            handleLoadMore()
          }
        }, 16) // ~60fps debouncing
      })
  ...
  const handleSendMessage = useCallback(
    async (prompt: string) => {
      ...
      try {
        ...
        console.debug("Workflow started", { workflowId, threadId: newThreadId, messageId })
      } catch (error) {
  ...
  const handleAbortStream = useCallback(() => {
    ...
    if (streams && Array.isArray(streams) && streams.length > 0) {
      const activeStream = streams.find((stream) => stream.streaming || stream.status === "pending")
      if (activeStream) {
  ...
  const isStreaming = useMemo(() => {
    ...
    if (streams && Array.isArray(streams) && streams.length > 0) {
      ...
      const hasActiveStream = streams.some((stream) => stream.streaming || stream.status === "pending")
    ...
    const hasStreamingMessage = recentMessages.some((m) => {
      ...
      if (toolParts.length > 0) {
        ...
        const allToolsDone = toolParts.every((p: any) => p.state === "output-available")
        if (allToolsDone) {
          // Check if there's text content being generated
          const hasTextPart = (m as any).parts?.some((p: any) => p.type === "text")
  ...
  useEffect(() => {
    ...
    const hasAssistantContent = messages.results?.some(
      (m) => (m as any).role === "assistant" && (m as any).parts && (m as any).parts.length > 0,
    )
  ...
  return (
    <>
      {/* Messages area */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-3" onScroll={handleScroll}>
