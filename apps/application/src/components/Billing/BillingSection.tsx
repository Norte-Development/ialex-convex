...
export function BillingSection({ teamId }: BillingSectionProps) {
  ...
  const portal = useAction((api as any).billing.subscriptions.portal);

  // Use the upgrade hook for handling plan upgrades
  const { upgradeToIndividual, upgradeToTeam, upgradeToTeamAutoCreate, isUpgrading } = useUpgrade({
    teamId,
  });
  ...
  const handleSelectPlan = async (selectedPlan: PlanType) => {
    ...
    if (selectedPlan === "premium_individual") {
      await upgradeToIndividual();
    } else if (selectedPlan === "premium_team") {
      // Check if user is free and trying to upgrade to team plan
      if (plan === "free") {
        // Show dialog explaining they need Premium Individual first
  ...
  const handleTeamUpgradeFromDialog = async () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <TeamUpgradeDialog
        ...
        onUpgradeTeamAutoCreate={handleTeamUpgradeFromDialog}
        isUpgrading={isUpgrading}
